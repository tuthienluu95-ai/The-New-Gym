...
export async function buildReport(sb, tu, den) {
  ...
  const { data: rows } = await sb.from('cham_cong')
    .select('nv_id, ngay, gio_vao, gio_ra, trang_thai, lich_lop_id, so_hoc_vien, nhan_vien!nv_id ( ma_nv, ho_ten, thu_lao ), clubs!club_id ( ten_club ), lich_lop!lich_lop_id ( ten_lop, gio_bat_dau, gio_ket_thuc )')
  ...
  const hvList = (rows || [])
    .filter((r) => typeof r.so_hoc_vien === 'number')
  ...
  return { list, totals, daily, tu, den, missed: missedList.length, missedList, hvList };
}
