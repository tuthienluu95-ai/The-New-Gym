...
export function fmtTime(v) {
  if (!v) return '';
  return new Date(v).toLocaleTimeString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh', hour: '2-digit', minute: '2-digit', hour12: false });
}

export function fmtDateTime(v) {
  if (!v) return '';
  return new Date(v).toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh', day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false });
}
...
export function vnMinutesOf(ts) {
  if (!ts) return null;
